...
if (isset($_GET['anular_factura'])) {
  $conexion->query("UPDATE facturas set status=0 where id_factura=$id_factura");
  header('location:facturas.php?status_anu');
} if (isset($_GET['activar_factura'])) {
  $conexion->query("UPDATE facturas set status=1 where id_factura=$id_factura");
  header('location:facturas.php?status_act');
}
 ?>
...
<table border="0" width="101%" align="center">
  <tr>
   ...
   <td><center> <b id="titulo_header"> DETALLES FACTURAS</b></center>
   </td>
...
<input type="text"disabled class="tamano"  id="reloj"name="reloj" size="10">
 ...
 <?php $sql=$conexion->query("SELECT t1.id_factura,t2.nombre,t2.apellido,DATE_FORMAT(t1.fecha_venta , '%d/%m/%Y')as fecha_venta,
  t1.hora_venta,t1.status,t1.id_cliente,t2.cedula
FROM facturas t1
INNER JOIN clientes t2 ON t2.id_cliente=t1.id_cliente
WHERE t1.id_factura='$codigo_fact'");
$fila=$sql->fetch_array(); 

$sql2=$conexion->query("SELECT t1.nombre_pro,t2.precioXu,t2.cant,t2.total
FROM productos t1
INNER JOIN detalles_facturas t2 ON t2.id_producto=t1.id_producto
WHERE t2.id_factura='$codigo_fact'");

?>

</td>
<td width="100"> <a href="facturas.php" class="btn btn-default"> <span class="glyphicon glyphicon-arrow-left" aria-hidden="true"></span> Regresar</a></td>
<?php if ($fila['status']==1) {
  echo '<th width="135"><a href="detalles_factura.php?anular_factura&id_factura='.$fila[0].'" class="btn btn-danger dropdown-toggle"> <span class="glyphicon glyphicon-remove"></span>&nbsp;Anular Factura</a>
';
}else{ echo '<th width="140"><a href="detalles_factura.php?activar_factura&id_factura='.$fila[0].'" class="btn btn-success dropdown-toggle"> <span class="glyphicon glyphicon-ok"></span>&nbsp;Activar Factura</a>
'; } ?>
     ...
     <td width="150"><?php echo '  <a href="../reportes/factura.php?id='.$fila[0].'&codigo_facturacion" id="bold" class="btn btn-warning"> <span class="glyphicon glyphicon-print"></span>&nbsp;Descargar PDF</a>';?></td>
  </tr>
</table>
  </div>
...
<table class="table table-striped table-condensed table-bordered">
   <tr>
     ...
     <td id="titulo_pro" class="bold texto_center">CLIENTE:</td>
      <td ><input type="text" disabled="" id="apellido_emp" class="form-control input-personalizado texto_center"  value="<?php echo $fila['nombre'].'&nbsp;'.$fila['apellido'] ?>"></td>
 
     <td id="titulo_pro" class="bold texto_center">FECHA:</td>
      <td ><input type="text" disabled="" id="apellido_emp" class="form-control input-personalizado texto_center"  value="<?php echo $fila['fecha_venta'] ?>"></td>
  
     <td id="titulo_pro" class="bold texto_center">HORA:</td>
      <td ><input type="text" disabled="" id="apellido_emp" class="form-control input-personalizado texto_center"  value="<?php echo $fila['hora_venta'] ?>"></td>
   </tr>
  
 </table>
 <table class="table table-striped table-condensed table-bordered">
 <tr><td id="titulo_pro" class="bold texto_center">PRODUCTO:</td>
    <td id="titulo_pro" class="bold texto_center">PRECIO X/U:</td>
    <td id="titulo_pro" class="bold texto_center">CANTIDAD:</td>
    <td id="titulo_pro" class="bold texto_center">TOTAL:</td>
    </tr>
    <tr>

      <?php 
    $acum=0;
      while ($prod=$sql2->fetch_array()) { ?>
      <tr>
      <td><?php echo $prod[0] ?></td>
      <td><?php echo $prod[1] ?></td>
      <td><?php echo $prod[2] ?></td>
      <td><?php echo $prod[3]; $acum+=$prod[3]; ?></td>
</tr>
      <?php } ?>
    </tr>
    <tr>
      ...
      <td class="Total bold"><?php echo $acum; ?></td>
    </tr>
